...
func TestValidateHandler(c echo.Context) error {
  ...
  if err != nil { log.Println(err); return echo.NewHTTPError(400, "Unable to get ead") }
  ...
  //get session for aw
  aw_session, err := aw.GetSession(c, true)
  ...
  //validate
  response, err := aw.Validate(aw_session, boundary, form)
